...
public class LanguagesController(IMediator mediator) : ControllerBase
{
    ...
    [HttpPatch]
    public async Task<ListResponse<LanguageDto>> GetLanguagesAsync(GetLanguageListQuery request, CancellationToken cancellation) => await _mediator.Send(request, cancellation);
    ...
    [HttpPut("language")]
    public async Task<Response<int>> UpdateLanguagesAsync(UpdateLanguageCommand request, CancellationToken cancellation) => await _mediator.Send(request, cancellation);
}